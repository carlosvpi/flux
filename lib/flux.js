class Flux {
  ...
  delay (ms) {
    return new Flux(this._value, (publish, end) => {
      this.forEach(value => {
        setTimeout(() => publish(value), ms)
      })
      this.last(() => setTimeout(() => end(), ms))
    })
  }
}
...
Flux.aggregate = (structure) => {
  ...
  for (let key in structure) {
    if (structure.constructor === Array) {
      aggregator.push(structure[key])
    } else {
      aggregator.set(key, structure[key])
    }
  }
  return aggregator
}
...
class Aggregator extends Flux {
  ...
  push (flux) {
    ...
    this._structureSubscriptions.push(flux.forEach(value => {
      const index = this._structure.indexOf(flux)
